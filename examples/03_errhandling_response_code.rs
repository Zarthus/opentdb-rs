...
fn main() {
    ...
    let rq: ApiRequest = opentdb::builder().token(token.to_string()).build();
    ...
    match rs.response_code {
        ...
        ResponseCode::TokenNotFound => {
            ...
            let _token = opentdb::session_new(None).unwrap();
        ...
        ResponseCode::Unknown => {
            // This response may exist if the API has introduced new codes but they are not
            // yet mapped to the library. You can deal with them yourself here, panic,
